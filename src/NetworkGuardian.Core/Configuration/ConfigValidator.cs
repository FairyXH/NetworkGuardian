...
public sealed class ConfigValidator
{
    ...
    public IReadOnlyList<string> Normalize(GuardianConfig config)
    {
        ...
        var enabledEndpoints = config.ProbeEndpoints.Count(e => e.Enabled);
        config.Probe.RequiredSuccessCount = Clamp(config.Probe.RequiredSuccessCount, 1, Math.Max(1, enabledEndpoints), "probe.requiredSuccessCount", issues);

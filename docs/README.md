...
当前交付形态是 **Native AOT 单文件便携版**（`release\NetworkGuardian.exe`，7.82 MB，自绘 Win32 UI，提权助手已内置；`release\` 为发布目录，已在 `.gitignore` 内）。
...
| 命令 | 断言 |
| --- | --- |
| `tools\build-portable.ps1` | 构建 0 警告 + 196 测试通过 + 单文件 ≤ 8 MB（包内只允许一个 exe）+ 无 PDB |
| `tools\verify-portable.ps1` | 中性目录 + 干净环境启动、窗口类存在、**窗口线程应答消息**、日志/配置副作用、**GUI 实例运行时 `--helper` 仍应答并真实执行**、关闭链路、无残留（35 项） |

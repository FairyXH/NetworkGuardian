...
| 安全选网 | 默认只连接 Windows 已保存的 Profile 或软件凭据库中由用户明确添加的网络 |
...
| Wi-Fi 凭据库 | DPAPI 加密个人/企业 Wi-Fi 密码，按需为每块网卡生成 Profile 和 EAP 用户凭据 |
...
4. 在“网络凭据库”中从周围网络添加 Wi-Fi，或手动录入 SSID、安全类型和凭据。
...
- **网络凭据库**：管理开放网、个人密码网与企业 Wi-Fi；密码由 DPAPI 保护，并可跨本机网卡复用。
...
候选必须满足：存在 Windows Profile 或可用的凭据库条目、不在黑名单、符合 SSID 白/黑名单、驱动报告可连接；凭据库网络还必须具备所需密码且未耗尽本轮重试次数。
...
软件维护独立于 Windows Profile 的 `wifi-networks.json`，支持 WPA/WPA2/WPA3-Personal、WEP、开放网络、Enhanced Open（OWE），以及企业级 802.1X/EAP 网络。用户可以从周围扫描结果直接添加，也可以手动创建隐藏网络。连接时软件会把同一份凭据写成目标网卡自己的 Windows Profile，因此更换或新增无线网卡后无需重新录入密码。
...
- 个人网络密码写入即将应用的 Windows Profile；库文件中只保存 DPAPI 密文。
- 企业网络支持生成 PEAP + MSCHAPv2 和 EAP-TLS 配置。

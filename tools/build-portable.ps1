...
#
# The size gate fails the build when the exe exceeds -MaxExeMb (default 10.0 MB).
#
...
#   7.87 MB  289ca18  before the library
#   7.98 MB  a8e6ce0  + library models / config / profile XML and EAP user-data builders
#   7.98 MB  06cc66d  + Windows profile writer, vault, DPAPI (still unreferenced, so trimmed away)
...
# connection-table/process matching support brings the package below 10 MB; this gate keeps explicit headroom.

param(
    [string]$Configuration = 'Release',
    [string]$OutputDir = 'release',
    [double]$MaxExeMb = 10.0,
    [switch]$SkipTests,
    [switch]$Zip
)
